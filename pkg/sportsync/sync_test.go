...
func TestEventDataChanged(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	t.Run("no change", func(t *testing.T) {
		g.Expect(EventDataChanged(base(), base())).Should(gomega.BeFalse())
	})

	t.Run("status changed", func(t *testing.T) {
		updated := base()
		updated.Status = model.SportsEventStatusInProgress
		g.Expect(EventDataChanged(base(), updated)).Should(gomega.BeTrue())
	})
	...
	t.Run("nil to non-nil", func(t *testing.T) {
		...
		g.Expect(EventDataChanged(existing, updated)).Should(gomega.BeTrue())
	})

	t.Run("both nil scores", func(t *testing.T) {
		existing := &model.SportsEvent{Status: model.SportsEventStatusScheduled}
		updated := &model.SportsEvent{Status: model.SportsEventStatusScheduled}
		g.Expect(EventDataChanged(existing, updated)).Should(gomega.BeFalse())
	})
}
