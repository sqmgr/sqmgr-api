...
func (m *Model) UpsertSportsEvent(ctx context.Context, q Queryable, event *SportsEvent) error {
	...
	const query = `
		...
		ON CONFLICT (espn_id) WHERE espn_id IS NOT NULL DO UPDATE SET
			...
			home_q1 = COALESCE(EXCLUDED.home_q1, sports_events.home_q1),
			home_q2 = COALESCE(EXCLUDED.home_q2, sports_events.home_q2),
			home_q3 = COALESCE(EXCLUDED.home_q3, sports_events.home_q3),
			home_q4 = COALESCE(EXCLUDED.home_q4, sports_events.home_q4),
			home_ot = COALESCE(EXCLUDED.home_ot, sports_events.home_ot),
			away_q1 = COALESCE(EXCLUDED.away_q1, sports_events.away_q1),
			away_q2 = COALESCE(EXCLUDED.away_q2, sports_events.away_q2),
			away_q3 = COALESCE(EXCLUDED.away_q3, sports_events.away_q3),
			away_q4 = COALESCE(EXCLUDED.away_q4, sports_events.away_q4),
			away_ot = COALESCE(EXCLUDED.away_ot, sports_events.away_ot),
