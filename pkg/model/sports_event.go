...
func (e *SportsEvent) IsPeriodComplete(setType NumberSetType) bool {
	...
	case NumberSetTypeQ1:
		return isFinal || period >= 2
	case NumberSetTypeQ2, NumberSetTypeHalf:
		return isFinal || period >= 3
	case NumberSetTypeQ3:
		return isFinal || period >= 4
...
func (m *Model) EventsNeedingScoreUpdate(ctx context.Context) ([]*SportsEvent, error) {
	const query = `
		...
		FROM sports_events
		WHERE status = 'in_progress'
		   OR (status = 'scheduled' AND event_date::date = CURRENT_DATE)
		   OR (status != 'final' AND event_date >= NOW() - INTERVAL '7 days' AND event_date <= NOW())
		ORDER BY event_date ASC
	`
	rows, err := m.DB.QueryContext(ctx, query)
	if err != nil {
