...
func (u *User) JoinGrid(ctx context.Context, g *Pool) error {
	_, err := u.db.ExecContext(ctx, "INSERT INTO pools_users (pool_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", g.id, u.ID)
	return err
}

// IsMemberOf will return true if the user belongs to the grid
func (u *User) IsMemberOf(ctx context.Context, g *Pool) (bool, error) {
	// user is the admin
	if u.ID == g.userID {
		return true, nil
	}
	...
	row := u.db.QueryRowContext(ctx, "SELECT true FROM pools_users WHERE pool_id = $1 AND user_id = $2", g.id, u.ID)
...
func (u *User) IsAdminOf(ctx context.Context, g *Pool) bool {
