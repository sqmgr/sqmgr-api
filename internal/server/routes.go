...
func (s *Server) setupRoutes() {
	...
	// Pool manager routes — require pool manager privileges
	authPoolManagerRouter := authPoolRouter.NewRoute().Subrouter()
	authPoolManagerRouter.Use(s.poolManagerHandler)
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}").Methods(http.MethodPost).Handler(s.postPoolTokenEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/grid/{id:[0-9]+}").Methods(http.MethodPost).Handler(s.postPoolTokenGridIDEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/season").Methods(http.MethodPost).Handler(s.postPoolTokenSeasonEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/invitetoken").Methods(http.MethodGet).Handler(s.getPoolTokenInviteTokenEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/log").Methods(http.MethodGet).Handler(s.getPoolTokenLogEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/members/emails").Methods(http.MethodGet).Handler(s.getPoolTokenMemberEmailsEndpoint())
	authPoolManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/squares/bulk").Methods(http.MethodPost).Handler(s.postPoolTokenSquaresBulkEndpoint())

	authPoolGridRouter := authPoolRouter.NewRoute().Subrouter()
	authPoolGridRouter.Use(s.poolGridHandler)
	authPoolGridSquareManagerRouter := authPoolGridRouter.NewRoute().Subrouter()
	authPoolGridSquareManagerRouter.Use(s.poolGridSquareManagerHandler)
	authPoolGridSquareManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/grid/{id:[0-9]+}/square/{square_id:[0-9]+}/annotation").Methods(http.MethodPost).Handler(s.postPoolTokenGridIDSquareSquareIDAnnotationEndpoint())
	authPoolGridSquareManagerRouter.Path("/pool/{token:[A-Za-z0-9_-]+}/grid/{id:[0-9]+}/square/{square_id:[0-9]+}/annotation").Methods(http.MethodDelete).Handler(s.deletePoolTokenGridIDSquareSquareIDAnnotationEndpoint())
	...
	adminRouter.Path("/admin/user/{id:[0-9]+}/joined-pools").Methods(http.MethodGet).Handler(s.getAdminUserJoinedPoolsEndpoint())
	...
	adminRouter.Path("/admin/events/{id:[0-9]+}/refresh").Methods(http.MethodPost).Handler(s.postAdminEventRefreshEndpoint())
	adminRouter.Path("/admin/events/{id:[0-9]+}/override").Methods(http.MethodPost).Handler(s.postAdminEventOverrideEndpoint())
	adminRouter.Path("/admin/events/{id:[0-9]+}/override").Methods(http.MethodDelete).Handler(s.deleteAdminEventOverrideEndpoint())
	adminRouter.Path("/admin/pool/{token:[A-Za-z0-9_-]+}").Methods(http.MethodGet).Handler(s.getAdminPoolEndpoint())
	adminRouter.Path("/admin/pool/{token:[A-Za-z0-9_-]+}/members").Methods(http.MethodGet).Handler(s.getAdminPoolMembersEndpoint())
	adminRouter.Path("/admin/pool/{token:[A-Za-z0-9_-]+}/activity").Methods(http.MethodGet).Handler(s.getAdminPoolActivityEndpoint())
	adminRouter.Path("/admin/pool/{token:[A-Za-z0-9_-]+}/squares").Methods(http.MethodGet).Handler(s.getAdminPoolSquaresEndpoint())
	adminRouter.Path("/admin/pool/{token:[A-Za-z0-9_-]+}/action").Methods(http.MethodPost).Handler(s.postAdminPoolActionEndpoint())
	adminRouter.Path("/admin/analytics/timeseries").Methods(http.MethodGet).Handler(s.getAdminTimeSeriesEndpoint())
	adminRouter.Path("/admin/analytics/fill-rates").Methods(http.MethodGet).Handler(s.getAdminFillRatesEndpoint())
	adminRouter.Path("/admin/analytics/breakdown").Methods(http.MethodGet).Handler(s.getAdminBreakdownEndpoint())
	adminRouter.Path("/admin/analytics/engagement").Methods(http.MethodGet).Handler(s.getAdminEngagementEndpoint())
	adminRouter.Path("/admin/analytics/top-creators").Methods(http.MethodGet).Handler(s.getAdminTopCreatorsEndpoint())
	adminRouter.Path("/admin/sports/status").Methods(http.MethodGet).Handler(s.getAdminSportsStatusEndpoint())
	adminRouter.Path("/admin/sports/sync-runs").Methods(http.MethodGet).Handler(s.getAdminSportsSyncRunsEndpoint())
	adminRouter.Path("/admin/sports/sync").Methods(http.MethodPost).Handler(s.postAdminSportsSyncEndpoint())
	adminRouter.Path("/admin/audit").Methods(http.MethodGet).Handler(s.getAdminAuditEndpoint())
