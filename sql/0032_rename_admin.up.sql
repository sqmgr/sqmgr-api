-- Rename admin columns to clarify site admin vs pool manager

BEGIN;

-- Rename users.is_admin to is_site_admin
ALTER TABLE users RENAME COLUMN is_admin TO is_site_admin;

-- Rename pools_users.is_admin to is_manager
ALTER TABLE pools_users RENAME COLUMN is_admin TO is_manager;

-- Recreate update_pool_square function with _is_manager parameter
...
CREATE FUNCTION update_pool_square(_id bigint, _state square_states, _claimant text, _user_id bigint,
                                   _remote_addr text, _note text, _is_manager boolean) RETURNS boolean
    LANGUAGE plpgsql
AS
$$
DECLARE
